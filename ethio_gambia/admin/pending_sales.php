...
<div id="wrapper">
 	...
 	<!--banner-->
        ...
        <form action="pending_sales.php" method="POST">
  ...
  if (isset($_POST['search'])) {
    $search = mysqli_real_escape_string($con,$_POST['search']);
      $sel = "SELECT * FROM sales_entri WHERE reference_no LIKE '%$search%' AND status=1 OR product_name LIKE '%$search%' AND status=1 OR product_code LIKE '%$search%' AND status=1 OR customer_name LIKE '%$search%' AND status=1 ORDER BY activity_date DESC" ;
     $run = mysqli_query($con, $sel);
  ...
  <thead>
    <tr>
      ...
      <th scope="col">Remaining Products</th>
  ...
  ?>
    <tr>
      ...
      <td><?php echo $qty;?></td>
      <td><?php echo $remaining_product;?></td>
      <td><?php echo $product_code;?></td>
      <td><?php echo $product_name;?></td>
      <td><?php echo $reference_no;?></td>
      <td><a class="btn btn-xs btn-success warning_4" href="edit_sale.php?id=<?php echo $id;?>">Finish Sale</a></td>
    </tr>
  <?php }}
  
   }else{
  $sel = "SELECT * from sales_entri where status=1 ORDER BY activity_date DESC"; 
  $run = mysqli_query($con, $sel); ?> 
     <table class="table table-striped">
  <thead>
    <tr>
