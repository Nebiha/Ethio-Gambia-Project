...
<div id="wrapper">
 	...
 	<!--banner-->
          ...
          <h3>
        Cap Production Entries
        </h3>
        <hr>

          <form action="super_cup_production.php" method="POST">
  ...
  if (isset($_POST['search'])) {
    $search = mysqli_real_escape_string($con,$_POST['search']);
      $sel = "SELECT * from production_entry WHERE employee_name LIKE '%$search%' OR machine_id LIKE '%$search%' OR cup_id LIKE '%$search%' OR shift LIKE '%$search%'OR product_code LIKE '%$search%' AND type='cap' order by activity_date DESC" ;
      $run = mysqli_query($con, $sel);
  ...
  ?><tbody>
    <tr>
      ...
      <td><?php echo $cup_id;?></td>
      <td><?php echo $product_code;?></td>
      <td><?php echo $product_qty;?></td>
      <td><?php echo $damaged_qty;?></td>
      <?php if ($edit==1) {
        echo '<td><a class="btn btn-xs btn-success warning_4" disabled="" data-toggle="tooltip" title="already edited" href="super_entry.php?id=<?php echo $id;?>">Edit</a></td>';
      }else {
        echo "<td><a class='btn btn-xs btn-success warning_4' href='super_entry.php?id=$id'>Edit</a></td>";
      } ?>
      <!-- <td><a class="btn btn-xs btn-success warning_4" href="edit_entry_cap.php?id=<?php echo $id;?>">Edit</a></td> -->
      <!-- <td><a class="btn btn-xs btn-danger" href="cup_production.php?id=<?php echo $id;?>">Delete</a></td> -->
    </tr>
   <?php }
    }
      }else{
  $sel = "SELECT * from production_entry where type='cap' order by activity_date DESC " ; $run = mysqli_query($con, $sel);
...
?>
  ...
  <tbody>
    <tr>
      ...
      <!-- <td><a class="btn btn-xs btn-success warning_4" href="super_entry.php?id=<?php echo $id;?>">Edit</a></td> -->
    <!--   <td><a class="btn btn-xs btn-danger" href="cup_production.php?id=<?php echo $id;?>">Delete</a></td> -->
    </tr>
   <?php }} ?>
  </tbody>
    ...
    if (isset($_GET['id'])) {
        ...
        $get_production_id = $_GET['id'];
            ...
            if ($run_delete) {
                ...
                echo "<script>window.open('cup_production.php','_self')</script>";
            }
    }
    ?> -->
 </div>
