...
<div id="wrapper">
 	...
 	<!--banner-->
        ...
        <form action="view_machine.php" method="POST">
                <div class="input-group input-group-in" style="padding-bottom: 0em;">
                    <input name="search" class="form-control2 input-search" placeholder="Search Machine..." type="text">
  ...
  if (isset($_POST['search'])) {
    $search = mysqli_real_escape_string($con,$_POST['search']);
      $sel = "SELECT * from machine WHERE machine_name LIKE '%$search%' OR machine_code LIKE '%$search%' OR machine_type LIKE '%$search%' " ;
     $run = mysqli_query($con, $sel);
  ...
  ?>
    ...
    <tr>
      ...
      <td><?php echo $machine_code;?></td>
      <td><?php echo $machine_type;?></td>
      <td><?php echo $machine_status;?></td>
        <td><a class="btn btn-xs btn-success warning_4" href="edit_machine.php?machine_id=<?php echo $machine_id;?>">Edit</a></td></tr>
    </tr>
      
  <?php }}
  
   }else{
  $sel = "SELECT * from machine order by machine_type ASC"; 
  $run = mysqli_query($con, $sel); ?> 
      <table class="table table-striped">
  <thead>
    <tr>
 ...
 <!-- <?php
    if (isset($_GET['machine_id'])) {
        ...
        $get_emp_id = $_GET['machine_id'];

            $Delete = "delete from machine WHERE machine_id='$get_emp_id'";
